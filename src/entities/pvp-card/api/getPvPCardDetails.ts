...
export async function getPvPCardDetails(roomId: number): Promise<PvPCardDetails | null> {
  try {
    const response = await proxyApiClient.get<GetPvPCardDetailsResponse>(
      `/proxy-api/pvp/rooms/${roomId}/result`,
    )
